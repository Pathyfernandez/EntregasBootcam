class Usuario:
    nombre_banco = "Primer Dojo Nacional"

    def __init__(self, name, email_address, saldo):
        self.nombre = name
        self.email = email_address
        self.saldo = saldo

    def hacer_depósito(self, cantidad):
        self.saldo += cantidad
        print("Depósito realizado. Saldo actual:", self.saldo)
        return self

    def retirar(self, cantidad):
        if cantidad <= self.saldo:
            self.saldo -= cantidad
            print("Retiro realizado. Saldo actual:", self.saldo)
        else:
            print("Saldo insuficiente para realizar el retiro.")
        return self

    def transferir(self, cantidad, destino):
        if cantidad <= self.saldo:
            self.saldo -= cantidad
            destino.hacer_depósito(cantidad)
            print("Transferencia realizada. Saldo actual:", self.saldo)
        else:
            print("Saldo insuficiente para realizar la transferencia.")
        return self

    def mostrar_balance_usuario(self):
        print("Nombre del usuario:", self.nombre)
        print("Saldo actual:", self.saldo)
        return self


# Actualiza tu asignación anterior para que los métodos de cada instancia estén encadenados
usuario1 = Usuario("Rodrigo", "rodrigo@codingdojo.com", 3000)
usuario1.mostrar_balance_usuario().hacer_depósito(1000).hacer_depósito(2000).hacer_depósito(500).retirar(600).retirar(1000).mostrar_balance_usuario()

usuario2 = Usuario("Jesus", "jesus@codingdojo.com", 5000)
usuario2.mostrar_balance_usuario().hacer_depósito(500).retirar(500).mostrar_balance_usuario()



usuario3 = Usuario("Pepita", "papita@codingdojo.com", 1000)
usuario3.mostrar_balance_usuario().hacer_depósito(10000).retirar(5000).retirar(500).retirar(800).mostrar_balance_usuario()

usuario1.transferir(600, usuario3)
usuario3.mostrar_balance_usuario()